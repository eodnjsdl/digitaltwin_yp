...
public class EgovUserManageServiceImpl extends EgovAbstractServiceImpl implements EgovUserManageService {
	...
	/** 
	 * @Description 회원가입 시에 해당 아이디에 대한 스키마 생성
	 * @Author 플랫폼개발부문 DT솔루션 정상혁
	 * @Date 2022.03.24
	 */
	@Override
	public void createUserSchema(String schemaNm) {
		 userManageDAO.createUserSchema(schemaNm);
	}

	@Override
	public void deleteUserSchema(String checkedIdForDel) throws Exception {
		String schemaNm = "";
		...
		for (int i=0; i<delId.length ; i++){
			...
			schemaNm =  Globals.CSV_DB_NAME + delId[i].toLowerCase();
			userManageDAO.deleteUserSchema(schemaNm);
		}
