...
class GraphicToolEditor {
  ...
  bindEvents() {
    ...
    // 심볼 직접 등록
    $(".drawingTabPoint .drawing-file", that.selector).on(
      "change",
      function () {
        const node = $(this);
    ...
    // 직접 등록 심볼 선택
    $(".drawingTabPoint .custom-container", that.selector).on(
      "click",
      "li",
      function () {
        const node = $(this);
    ...
    // 선 모양
    $(".drawingTabLine .strokeLineDash", that.selector).on(
      "change",
      function () {
        if (that.feature) {
    ...
    // 선 시작화살표
    $(".drawingTabLine .strokeStartArrow", that.selector).on(
      "click",
      function () {
        if (that.feature) {
    ...
    // 선 끝화살표
    $(".drawingTabLine .strokeEndArrow", that.selector).on(
      "click",
      function () {
        if (that.feature) {
    ...
    // 면 - 선 색상
    $(".drawingTabPolygon .strokeColor", that.selector).on(
      "change",
      function () {
        if (that.feature) {
          that.feature.set("strokeColor", $(this).val());
    ...
    // 면 - 선 모양
    $(".drawingTabPolygon .strokeLineDash", that.selector).on(
      "change",
      function () {
        if (that.feature) {
          that.feature.set("strokeLineDash", $(this).val());
    ...
    // 정렬
    $(".drawingTabText [name=textAlign]", that.selector).on(
      "click",
      function () {
        if (that.feature) {
    ...
    $(".btn_cancel", that.selector).on("click", function () {
      ...
      that.cancel(data);
      if(app2D==false || app2D==null){removeGrphLayer();}
    });
