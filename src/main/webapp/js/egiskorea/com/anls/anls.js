...
//						var modulName ; 
						if(data[i] == 'M_ROV_ANLS') {//조망권분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">조망권분석(3D)</li>';
						}else if(data[i] == 'M_SUHN_ANLS') {//일조권분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">일조권분석(3D)</li>';
						}else if(data[i] == 'M_VSBL_ANLS') {//가시권분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">가시권분석(3D)</li>';							
						}else if(data[i] == 'M_SLOPE') {//경사분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">경사분석(3D)</li>';
						}else if(data[i] == 'M_AI_IMAGE') {//ai영상분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">AI영상분석(3D)</li>';
						}else if(data[i] == 'M_TPPH_SECT'){//지형단면도
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">지형단면도(3D)</li>';
						}else if(data[i] == 'M_UNDG_FCTY_SECT') {//지하시설단면도
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">지하시설단면도</li>';
						}else if(data[i] == 'M_SPCE_ANLS') {//공간분석
							html +='<li><button type="button" class="dataPopup" name='+data[i]+' onclick="module.select(\''+data[i]+'\')">공간분석</li>';
						}else {
