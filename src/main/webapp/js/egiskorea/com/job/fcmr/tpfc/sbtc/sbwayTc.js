...
function setSubwayTrackListGrid() {
	...
	columns: [
	    ...
	    {key: "kor_sbr_nm",		label: "지하철노선명(한글)",		width: "*"},
	    {key: "opert_de",		label: "작업일시",				width: "*"},
	    {key: "sub_rlw_sn",		label: "지하철노선 일련번호",		width: "*"}
	],
    });
}
 ...
 * @returns
 */
function getWfsSubwayTrackListData() {
	...
	var geoArry = dtmap.util.readGeoJson(data);
    	...
    	function setEmdCd(geoArry) {
    	    ...
    	    return setSubwayTrackListData(0, geom);
    	}
    });
}
 ...
 * @returns
 */
function downloadExcelSubwayTrack() {
    ui.loadingBar("show");
    // 엑셀 다운로드를 위한 grid 생성
	...
	columns: [
	    ...
	    {key: "kor_sbr_nm",		label: "지하철노선명(한글)",		},
	    {key: "opert_de",		label: "작업일시",				},
	    {key: "sub_rlw_sn",		label: "지하철노선 일련번호",		}
	]
    });
	...
	excelGrid.exportExcel("EXPORT_지하철선로.xls");
