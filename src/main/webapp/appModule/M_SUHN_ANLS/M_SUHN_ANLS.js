...
var M_SUHN_ANLS = {
    ...
    timeStep: 1,
    destroy: function () {
        M_SUHN_ANLS.Analysis.clearShadow();
        if (M_SUHN_ANLS.itv) {
            clearTimeout(M_SUHN_ANLS.itv)
            M_SUHN_ANLS.itv = undefined;
        }
    },
    init: function () { // 초기화
        //정북방향보기
        ...
        clearTimeout(M_SUHN_ANLS.itv);
        M_SUHN_ANLS.Analysis = Module.getAnalysis();
        M_SUHN_ANLS.Analysis.clearShadow();
    ...
    controlPlay: function () { // 재생 // 그림자생성 -초기화로 다시 그려줌
        ...
        M_SUHN_ANLS.itv = setTimeout(function () {
            ...
            //그림자 생성
            M_SUHN_ANLS.Analysis.createShadow(year, month, date, hour, minute);
