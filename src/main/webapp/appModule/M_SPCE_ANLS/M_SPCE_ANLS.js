...
var M_SPCE_ANLS = {
    ...
    highlight: function () {
        ...
        if (emdCode) {
            //읍면동 피쳐로 필터링
            store.emd.getData().done((features) => {
                const findFeature = features.find((feature) => feature.get("emd_cd") === emdCode);
                const geometry = findFeature.getGeometry().clone();
                if (dtmap.crs !== 'ESPG:5179') {
                    geometry.transform('EPSG:5179', dtmap.crs);
                }
                param.geometry = geometry;
                dtmap.wfsGetFeature(param).then(function (e) {
                    dtmap.vector.clear();
                    if (e.features.length === 0) {
                        return toastr.warning('검색결과가 없습니다.');
                    }
            ...
            if (dataId === "tgd_spot_bridge" || dataId === 'tgd_spsb_statn') {
                //전국데이터 인경우 시군구 코드로 필터링
                param.filter = 'sig_cd = 41830';
            }
            dtmap.wfsGetFeature(param).then(function (e) {
                dtmap.vector.clear();
                if (e.features.length === 0) {
