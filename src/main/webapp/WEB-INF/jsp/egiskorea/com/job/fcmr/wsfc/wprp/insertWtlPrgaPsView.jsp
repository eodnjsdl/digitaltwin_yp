...
<style type="text/css">
	.popup-panel.popup-sub .insert-wtlPrgaPs-popup-close {
	    top: 0;
	    right: 0;
	    width: 39px;
	    height: 39px;
...
<!-- 업무 > 시설관리 > 상수수도시설 > 수압계 등록하기-->

       	<div class="popup-header">수압계 등록하기</div>
           <div class="popup-body">
               <div class="sub-popup-body">
                   <div class="data-write-wrap" style="height: 100%;">
                       <div class="scroll-y">
                           <form id="insertWtlPrgaPsForm" method="post">
										...
										<tr>
	                                       	...
	                                       	<td>
	                                          	<select name="ftr_cde" class="form-select">
	                                          		<option value="SA121" selected="selected">수압계</option>
	                                          	</select>
	                                       	</td>
                       ...
                       <div class="position-bottom btn-wrap">
                           <div>
                           	    <button type="button" class="btn basic bi-edit btn_add" onclick="insertWtlPrgaPs();">등록</button>
                           		<button type="button" class="btn basic bi-cancel btn_cancel" onclick="cancelInsertWtlPrgaPs()">취소</button>
                           </div>
                       </div>
                   </div>
               </div>
           </div>
           <button type="button" class="insert-wtlPrgaPs-popup-close" title="닫기" onclick="cancelMode();"></button>

<!-- 업무 > 시설관리 > 상수수도시설 > 수압계 등록하기 end -->
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//3d 일때 지도 추가 버튼 삭제 
		if(dtmap.mod == "3D"){
			if($("#insertWtlPrgaPsForm .btn-select-map").css("display") != 'none'){
				$("#insertWtlPrgaPsForm .btn-select-map").hide();
			}
		}
        ...
        $(".btn-select-map", this).on("click", function () {
     	...
     	//등록창 닫기
     	$(".popup-panel .insert-wtlPrgaPs-popup-close").on("click", function () {
             cancelInsertWtlPrgaPs();
     	});
     	
	});
	
	//취소 버튼 동작
	function cancelInsertWtlPrgaPs() {
		//console.log("cancelInsertWtlPrgaPs()");
		
		$(".insert-wtlPrgaPs-popup-close").closest('.popup-panel').removeClass('opened');
        // 초기화 (지도)
        dtmap.draw.dispose();
        dtmap.draw.clear();
