				...
				<div class="sttn-list-wrap scroll-y">
					<ul class="sttn-list">
						<c:forEach items="${thrghSttnList}" var="thrghSttn" varStatus="status">
							<li>
								<a href="javascript:drawCrdnt('<c:out value="${thrghSttn.xCrdnt}" />', '<c:out value="${thrghSttn.yCrdnt}" />', '<c:out value="${thrghSttn.sttnNm}" />')" data-popup="sttn-detail">
									<span class="sttnNm">${thrghSttn.sttnNm}</span>
									<span class="sttnId">${thrghSttn.sttnId}</span>
								</a>
							</li>
						</c:forEach>
					</ul>
