...
// 변수선언
var subwayStationVO = ${subwayStationVO}
var korSubNm = subwayStationVO.korSubNm
var emdKorNm = subwayStationVO.emdKorNm
var trfcSelect = subwayStationVO.trfcSelect
var trfcAreaDrawing = subwayStationVO.trfcAreaDrawing
var bufferCnt = subwayStationVO.bufferCnt
...
// 지하철역사 최초 페이지
function fn_select_subway_station_list(){
	...
	aj_selectSubwayStationList($("#searchForm")[0], searchType.val());
}

// 지하철역사 페이징
function fn_select_subway_station_linkPage(pageNo){
...
// 지하철역사 상세보기
function fn_select_subway_station(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('subwayStation', gid)
}
					...
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							<div class="top-search">
								<select name="selectBoxTrfc" id="selectBoxTrfc" class="form-select">
									...
									<option value="subwayStation" selected>지하철역사</option>
									<option value="bridge">교량</option>
									<option value="overpass">고가도로</option>
									<option value="tunnel">터널</option>
								</select>
							</div>
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_subway_station_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" class="form-control" id="korSubNm" name="korSubNm" placeholder="지하철선로명" value="<c:out value='${searchVO.korSubNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectSubwayStationExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="swstList" varStatus="status">
												<tr id="<c:out value="${swstList.gid}" />" data-gid="<c:out value="${swstList.gid}" />" data-lon="<c:out value="${swstList.lon}" />" data-lat="<c:out value="${swstList.lat}" />"
												 onclick="fn_select_subway_station('<c:out value="${swstList.gid}"></c:out>', '<c:out value="${swstList.lon}"></c:out>', '<c:out value="${swstList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${swstList.sigCd}"></c:out></td>
													<td><c:out value="${swstList.subStaSn}"></c:out></td>
													<td><c:out value="${swstList.korSubNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
													<fmt:parseDate value="${swstList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_subway_station_linkPage"/>
								</div>
							</div>
						</div>
					</div>
