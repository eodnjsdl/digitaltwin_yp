				...
				<div class="tab-cont constructionInquiry on" id="divConstructionInquiry">
					...
					<div class="tabBoxDepth2-wrap marB20">
						...
						<div class="tab-cont constructionInfo01">
							<form:form name="searchInquiryForm" id="searchInquiryForm" method="post" onsubmit="fn_selectInquiry_linkPage(1); return false;">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													...
													<div class="form-row">
														<div class="col-4"><input type="text" class="form-control" name="chpsnNm" id="chpsnNm" placeholder="담당자명" value="<c:out value='${searchVO.chpsnNm}' />"></div>
														<div class="col-8"><input type="text" class="form-control" name="cntrkNm" id="cntrkNm" placeholder="공사명을 입력해주세요." value="<c:out value='${searchVO.cntrkNm}' />"></div>
													</div>
												</td>
											</tr>
										</tbody>
								...
								<div class="btn-wrap">
									<div><button type="submit" class="btn type01 search" name="cwiSearch" id="cwiSearch">조회</button></div>
								</div>
							</form:form>
						</div>
									...
									<table class="srch-tbl">
										...
										<tbody>
											...
											<tr>
												...
												<td><input type="text" class="form-control w-100 " placeholder="0" name="radius" id="radius"> <sub>m</sub></td>
											</tr>
										</tbody>
									</table>
								</div>
								<div class="btn-wrap">
									<div><button type="button" class="btn type01 search" name="cwiSearch02" id="cwiSearch02" >조회</button></div>
								</div>
					...
					<div class="bbs-list-wrap" style="height: 432px;"><!-- pagination 하단 고정을 위해 반드시 필요 -->
								...
								<table class="bbs-list">
									...
									<tbody>
										<c:forEach items="${resultList}" var="cwiList" varStatus="status">
											<tr name="tdCwiDtl" data-cwiid='<c:out value="${cwiList.cntrkPrrngId}" ></c:out>' data-lon='<c:out value="${cwiList.lon}" ></c:out>' data-lat='<c:out value="${cwiList.lat}" ></c:out>'>
												<c:forEach items="${codeList}" var="codeList" varStatus="status">
												...
												<td><c:out value="${cwiList.plnYear}"></c:out>년 <c:out value="${cwiList.plnQu}"></c:out></td>
						...
						<div class="pagination">
							<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_selectInquiry_linkPage"/>
						</div>
					</div>
				</div>
				<!-- //공사정보 조회 -->
