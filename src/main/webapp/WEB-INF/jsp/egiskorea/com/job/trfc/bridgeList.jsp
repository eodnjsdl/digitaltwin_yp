...
// 변수선언
var bridgeVO = ${bridgeVO}
var korBriNm = bridgeVO.korBriNm
var emdKorNm = bridgeVO.emdKorNm
var trfcSelect = bridgeVO.trfcSelect
var trfcAreaDrawing = bridgeVO.trfcAreaDrawing
var bufferCnt = bridgeVO.bufferCnt
...
// 교량 최초 페이지
function fn_select_bridge_list(){
	...
	aj_selectBridgeList($("#searchForm")[0], searchType.val());
}

// 교량 페이징
function fn_select_bridge_linkPage(pageNo){
...
// 교량 상세보기
function fn_select_bridge(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('bridge', gid)
}
					...
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							<div class="top-search">
								<select name="selectBoxTrfc" id="selectBoxTrfc" class="form-select">
									...
									<option value="bridge" selected>교량</option>
									<option value="overpass">고가도로</option>
									<option value="tunnel">터널</option>
								</select>
							</div>
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_bridge_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" class="form-control" id="korBriNm" name="korBriNm" placeholder="교량명" value="<c:out value='${searchVO.korBriNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectBridgeExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="brdgList" varStatus="status">
												<tr id="<c:out value="${brdgList.gid}" />" data-gid="<c:out value="${brdgList.gid}" />" data-lon="<c:out value="${brdgList.lon}" />" data-lat="<c:out value="${brdgList.lat}" />"
												onclick="fn_select_bridge('<c:out value="${brdgList.gid}"></c:out>', '<c:out value="${brdgList.lon}"></c:out>', '<c:out value="${brdgList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${brdgList.sigCd}"></c:out></td>
													<td><c:out value="${brdgList.bridgeSn}"></c:out></td>
													<td><c:out value="${brdgList.korBriNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
													<fmt:parseDate value="${brdgList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_bridge_linkPage"/>
								</div>
							</div>
						</div>
					</div>
