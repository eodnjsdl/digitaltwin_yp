...
// 변수선언
var overpassVO = ${overpassVO}
var korOveNm = overpassVO.korOveNm
var emdKorNm = overpassVO.emdKorNm
var trfcSelect = overpassVO.trfcSelect
var trfcAreaDrawing = overpassVO.trfcAreaDrawing
var bufferCnt = overpassVO.bufferCnt
...
// 고가도로 최초 페이지
function fn_select_overpass_list(){
	...
	aj_selectOverpassList($("#searchForm")[0], searchType.val());
}

// 고가도로 페이징
function fn_select_overpass_linkPage(pageNo){
...
// 고가도로 상세보기
function fn_select_overpass(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('overpass', gid)
}
					...
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							<div class="top-search">
								<select name="selectBoxTrfc" id="selectBoxTrfc" class="form-select">
									...
									<option value="overpass" selected>고가도로</option>
									<option value="tunnel">터널</option>
								</select>
							</div>
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_overpass_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" id="korOveNm" name="korOveNm" class="form-control" placeholder="고가도로명" value="<c:out value='${searchVO.korOveNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectOverpassExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="ovpsList" varStatus="status">
												<tr id="<c:out value="${ovpsList.gid}" />" data-gid="<c:out value="${ovpsList.gid}" />" data-lon="<c:out value="${ovpsList.lon}" />" data-lat="<c:out value="${ovpsList.lat}" />"
												onclick="fn_select_overpass('<c:out value="${ovpsList.gid}"></c:out>', '<c:out value="${ovpsList.lon}"></c:out>', '<c:out value="${ovpsList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${ovpsList.sigCd}"></c:out></td>
													<td><c:out value="${ovpsList.oveSn}"></c:out></td>
													<td><c:out value="${ovpsList.korOveNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
<%-- 													<fmt:parseDate value="${ovpsList.opertDe}" var="dateValue" pattern="yyyyMMdd"/> --%>
<%-- 													<fmt:parseDate value="${ovpsList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/> --%>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
													<td><c:out value="${ovpsList.opertDe}"></c:out></td>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd HH:mm:ss"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_overpass_linkPage"/>
								</div>
							</div>
						</div>
					</div>
