...
// 변수선언
var subwayTrackVO = ${subwayTrackVO}
var korSbrNm = subwayTrackVO.korSbrNm
var emdKorNm = subwayTrackVO.emdKorNm
var trfcSelect = subwayTrackVO.trfcSelect
var trfcAreaDrawing = subwayTrackVO.trfcAreaDrawing
var bufferCnt = subwayTrackVO.bufferCnt
...
// 지하철선로 최초 페이지
function fn_select_subway_track_list(){
	...
	aj_selectSubwayTrackList($("#searchForm")[0], searchType.val());
}

// 지하철선로 페이징
function fn_select_subway_track_linkPage(pageNo){
...
// 지하철선로 상세보기
function fn_select_subway_track(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('subwayTrack', gid)
}
				...
				<div class="popup-body trfc">
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							...
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_subway_track_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" id="korSbrNm" name="korSbrNm" class="form-control" placeholder="철도역사명" value="<c:out value='${searchVO.korSbrNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectSubwayTrackExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="swtkList" varStatus="status">
												<tr id="<c:out value="${swtkList.gid}" />" data-gid="<c:out value="${swtkList.gid}" />" data-lon="<c:out value="${swtkList.lon}" />" data-lat="<c:out value="${swtkList.lat}" />"
												onclick="fn_select_subway_track('<c:out value="${swtkList.gid}"></c:out>', '<c:out value="${swtkList.lon}"></c:out>', '<c:out value="${swtkList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${swtkList.sigCd}"></c:out></td>
													<td><c:out value="${swtkList.subRlwSn}"></c:out></td>
													<td><c:out value="${swtkList.korSbrNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
													<fmt:parseDate value="${swtkList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_subway_track_linkPage"/>
								</div>
							</div>
						</div>
					</div>
