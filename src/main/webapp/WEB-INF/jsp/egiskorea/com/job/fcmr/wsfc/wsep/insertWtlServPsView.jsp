...
<style type="text/css">
	.popup-panel.popup-sub .insert-wtlServPs-popup-close {
	    top: 0;
	    right: 0;
	    width: 39px;
	    height: 39px;
           ...
           <div class="popup-body">
               <div class="sub-popup-body">
                   <div class="data-write-wrap" style="height: 100%;">
                       <div class="scroll-y">
                           <form id="insertWtlServPsForm" method="post">
                       ...
                       <div class="position-bottom btn-wrap">
                           <div>
                           	    <button type="button" class="btn basic bi-edit btn_add" onclick="insertWtlServPs();">등록</button>
                           		<button type="button" class="btn basic bi-cancel btn_cancel" onclick="cancelInsertWtlServPs()">취소</button>
                           </div>
                       </div>
                   </div>
               </div>
           </div>
           <button type="button" class="insert-wtlServPs-popup-close" title="닫기" onclick="cancelMode();"></button>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//3d 일때 지도 추가 버튼 삭제 
		if(dtmap.mod == "3D"){
			if($("#insertWtlServPsForm .btn-select-map").css("display") != 'none'){
				$("#insertWtlServPsForm .btn-select-map").hide();
			}
		}
	 ...
	 // 지도에서 선택 화면 호출
        $(".btn-select-map", this).on("click", function () {
     	...
     	//등록창 닫기
     	$(".popup-panel .insert-wtlServPs-popup-close").on("click", function () {
             cancelInsertWtlServPs();
     	});
     	
	});
	
	//취소 버튼 동작
	function cancelInsertWtlServPs() {
		//console.log("cancelInsertWtlServPs()");
		
		$(".insert-wtlServPs-popup-close").closest('.popup-panel').removeClass('opened');
        // 초기화 (지도)
        dtmap.draw.dispose();
        dtmap.draw.clear();
