...
// 변수선언
var railroadStationVO = ${railroadStationVO}
var korStaNm = railroadStationVO.korStaNm
var emdKorNm = railroadStationVO.emdKorNm
var trfcSelect = railroadStationVO.trfcSelect
var trfcAreaDrawing = railroadStationVO.trfcAreaDrawing
var bufferCnt = railroadStationVO.bufferCnt
...
// 철도역사 최초 페이지
function fn_select_railroad_station_list(){
	...
	aj_selectRailroadStationList($("#searchForm")[0], searchType.val());
}

// 철도역사 페이징
function fn_select_railroad_station_linkPage(pageNo){
...
// 철도역사 상세보기
function fn_select_railroad_station(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('railroadStation', gid)
}
				...
				<div class="popup-body trfc">
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							...
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_railroad_station_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" class="form-control" name="korStaNm" id="korStaNm" placeholder="철도역사명" value="<c:out value='${searchVO.korStaNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectRailroadStationExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="rrskList" varStatus="status">
												<tr id="<c:out value="${rrskList.gid}" />" data-gid="<c:out value="${rrskList.gid}" />" data-lon="<c:out value="${rrskList.lon}" />" data-lat="<c:out value="${rrskList.lat}" />"
												 onclick="fn_select_railroad_station('<c:out value="${rrskList.gid}"></c:out>', '<c:out value="${rrskList.lon}"></c:out>', '<c:out value="${rrskList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${rrskList.sigCd}"></c:out></td>
													<td><c:out value="${rrskList.rlrStaSn}"></c:out></td>
													<td><c:out value="${rrskList.korStaNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
													<fmt:parseDate value="${rrskList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_railroad_station_linkPage"/>
								</div>
							</div>
						</div>
					</div>
