				...
				<div class="tab-cont constructionSchedule on" id="divConstructionSchedule">
					<form:form name="searchScheduleForm" id="searchScheduleForm" method="post" onsubmit="fn_selectSchedule_linkPage(1); return false;">
						...
						<div class="srch-box">
							...
							<div class="btn-wrap">
								<div><button type="button" class="btn type01 search" name="cwsSearch" id="cwsSearch">조회</button></div>
							</div>
						</div>
					</form:form>
					<div class="bbs-top">
						<div class="bbs-list-num">조회결과 : <strong><c:out value="${resultCnt}"></c:out></strong>건</div>
						<div><button type="button" class="btn bi-write" id="cwsRegist">등록</button></div>
					</div>
								...
								<table class="bbs-list">
									...
									<tbody>
										...
										<c:forEach items="${resultList}" var="csList" varStatus="status">
											<tr name="tdCwsDtl" data-cwsid='<c:out value="${csList.cntrkPrrngId}" ></c:out>' data-lon='<c:out value="${csList.lon}" ></c:out>' data-lat='<c:out value="${csList.lat}" ></c:out>'>
												<%-- <td>
