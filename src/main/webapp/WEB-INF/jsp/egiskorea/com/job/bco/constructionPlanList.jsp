		...
		<div class="left-popup-body">
			<div class="tabBoxDepth1-wrap">
				...
				<div class="tab-cont constructionPlan on" id="divConstructionPlan">
					<form:form name="searchPlanForm" id="searchPlanForm" method="post" onsubmit="fn_selectPlan_linkPage(1); return false;">
							...
							<div class="srch-default">
								<table class="srch-tbl">
									...
									<tbody>
										...
										<tr>
											<th scope="row">공사명</th>
											<td><input type="text" class="form-control" placeholder="공사명을 입력해주세요." id="cntrkNm" name="cntrkNm" value="<c:out value='${searchVO.cntrkNm}' />"></td>
										</tr>
									</tbody>
								</table>
							</div>
							<div class="btn-wrap">
								<div><button type="button" class="btn type01 search" name="cplSearch">조회</button></div>
							</div>
						</div>
					</form:form>
					<div class="bbs-top">
					...
					<div class="bbs-list-wrap" style="height: 456px;"><!-- pagination 하단 고정을 위해 반드시 필요 -->
								...
								<table class="bbs-list">
									...
									<tbody>
										<c:forEach items="${resultList}" var="cpList" varStatus="status">
											<tr name="tdCwpDtl" id="tdCwpDtl" data-cpi='<c:out value="${cpList.cntrkPlnId}" />' data-lon='<c:out value="${cpList.lon}" />' data-lat='<c:out value="${cpList.lat}" />'>
												<%-- <td>
						...
						<div class="pagination">
							<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_selectPlan_linkPage"/>
						</div>
						<!-- <div class="pagination">
