...
// 변수선언
var tunnelVO = ${tunnelVO}
var tunKorNm = tunnelVO.tunKorNm
var emdKorNm = tunnelVO.emdKorNm
var trfcSelect = tunnelVO.trfcSelect
var trfcAreaDrawing = tunnelVO.trfcAreaDrawing
var bufferCnt = tunnelVO.bufferCnt
...
console.log("$('#tunKorNm').val() : " + $("#tunKorNm").val())
console.log("tunKorNm : " + tunKorNm)


if(tunKorNm != ''){
	$("#tunKorNm").val(tunKorNm)
}
if(emdKorNm != ''){
...
// 교량 최초 페이지
function fn_select_tunnel_list(){
	...
	aj_selectTunnelList($("#searchForm")[0], searchType.val());
}

// 교량 페이징
function fn_select_tunnel_linkPage(pageNo){
...
// 교량 상세보기
function fn_select_tunnel(gid, lon, lat){
	
	//cmmUtil.setCameraMove(lon, lat);
	rightSubPopupOpen('tunnel', gid)
}
				...
				<div class="popup-body trfc">
					<div class="bottom-popup-body bottom-popup-group">						
						...
						<div class="items search-area">
							...
							<form:form name="searchForm" id="searchForm" method="post" onsubmit="fn_select_tunnel_list(); return false;">
									...
									<div class="srch-default">
										<table class="srch-tbl">
											...
											<tbody>
												...
												<tr>
													<td colspan="2"><input type="text" class="form-control" id="tunKorNm" name="tunKorNm" placeholder="터널명" value="<c:out value='${searchVO.tunKorNm}' />"></td>
												</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="items data-area">
							<div class="bbs-top">
								...
								<div>
									<button type="button" class="btn basic bi-excel trfcExcelDownload" id="selectTunnelExcelList">엑셀저장</button>
...
<%-- 												<col style="width: 36px;"> --%>
											...
											<c:forEach items="${resultList}" var="tunlList" varStatus="status">
												<tr id="<c:out value="${tunlList.gid}" />" data-gid="<c:out value="${tunlList.gid}" />" data-lon="<c:out value="${tunlList.lon}" />" data-lat="<c:out value="${tunlList.lat}" />"
												onclick="fn_select_tunnel('<c:out value="${tunlList.gid}"></c:out>', '<c:out value="${tunlList.lon}"></c:out>', '<c:out value="${tunlList.lat}"></c:out>')">
<!-- 													<td><span class="form-checkbox"><span><input type="checkbox" name="" id="chk1" checked=""><label for="chk1"></label></span></span></td> -->
													<td><c:out value="${tunlList.sigCd}"></c:out></td>
													<td><c:out value="${tunlList.tunnelSn}"></c:out></td>
													<td><c:out value="${tunlList.tunKorNm}"></c:out></td>
													<!-- operDe가 String이여서 Date타입 변경 후 fmt:formatDate 사용 -->
													<fmt:parseDate value="${tunlList.opertDe}" var="dateValue" pattern="yyyyMMddHHmmss"/>
<%-- 													<td><fmt:formatDate value="${dateValue}" pattern="yyyy.MM.dd"/></td> --%>
								...
								<div class="pagination">
									<ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_tunnel_linkPage"/>
								</div>
							</div>
						</div>
					</div>
