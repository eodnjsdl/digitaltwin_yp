...
function aj_selectOpQna(qaId, pageIndex, searchCnd, searchWrd){
	$.ajax({
		type : "POST",
		url : "/com/opqna/selectOpQna.do",
		data : {
			qaId : qaId,
			pageIndex : pageIndex,
			searchCnd : searchCnd,
					...
					<div class="popup-body">
						<div class="bbs-popup-body">
							<div class="scroll-y" style="height: 520px;">
								<h3 class="cont-tit marT0">운영지원 수정하기</h3>
							...
							<div class="position-bottom btn-wrap">
								...
								<div>
									...
									<button type="button" class="btn basic bi-cancel" onclick="aj_selectOpQna('<c:out value="${qnaVO.qaId}"/>','<c:out value="${searchVO.pageIndex}"/>', '<c:out value="${searchVO.searchCnd}"/>', '<c:out value="${searchVO.searchWrd}"/>')">취소</button>
								</div>
							</div>
