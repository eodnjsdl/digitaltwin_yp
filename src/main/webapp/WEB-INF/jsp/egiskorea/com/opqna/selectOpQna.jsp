...
function aj_updateOpQnaView(){
	...
	$.ajax({
		type : "POST",
		url : "/com/opqna/updateOpQnaView.do",
		data : {
			qaId : qaId,
			pageIndex : pageIndex,
			searchCnd : searchCnd,
...
function aj_insertOpQnaAnswer(){
	...
	$.ajax({
		type : "POST",
		url : "/com/opqna/insertOpQnaAnswer.do",
		data : {
			qaId : qaId,
			answerCn : answerCn,
			pageIndex : pageIndex
...
//답변 수정
function aj_updateOpQnaAnswerView(){
	...
	$.ajax({
		type : "POST",
		url : "/com/opqna/updateOpQnaAnswerView.do",
		data : {
			qaId : qaId,
			pageIndex : pageIndex
		},
...
function aj_deleteOpQnaAnswer(){
	...
	if(confirm("<spring:message code="common.delete.msg" />")){	
		$.ajax({
			type : "POST",
			url : "/com/opqna/deleteOpQnaAnswer.do",
			data : {
				qaId : qaId,
				pageIndex : pageIndex
			},
					...
					<div class="popup-body">
						<div class="bbs-popup-body">
								...
								<div class="comments-panel "><!-- 댓글 없으면 noComments 추가 / 댓글 있으면 noComments 삭제 -->
											...
											<ul class="comments-list">
												<li>
													<div class="user-box">
														...
														<div>
															<button type="button" class="comments-btn edit" title="수정" onclick="aj_updateOpQnaAnswerView('<c:out value="${result.qaId}"/>');"></button>
															<button type="button" class="comments-btn delete" title="삭제" onclick="aj_deleteOpQnaAnswer('<c:out value="${result.qaId}"/>');" ></button>
														</div>
														</c:if>
													</div>
													<div class="comments">
									...
									<div class="comments-footer">
											...
											<div class="btn-wrap justify-content-end marT5">
												<div><button type="button" class="btn basic bi-comments" onclick="aj_insertOpQnaAnswer();">답글등록</button></div>								
											</div>
									</div>
									</c:if>
								</div>
							...
							<div class="position-bottom btn-wrap">
								<div> 
									<button type="button" class="btn basic bi-write2" onclick="aj_selectOpQnaList('<c:out value="${searchVO.pageIndex}"/>', '<c:out value="${searchVO.searchCnd}"/>', '<c:out value="${searchVO.searchWrd}"/>');">목록</button>
								</div>
								...
								<div class="position-absolute right">
									<button type="button" class="btn basic bi-edit" onclick="aj_updateOpQnaView();">수정</button>
									<button type="button" class="btn basic bi-delete" onclick="aj_deleteOpQna();">삭제</button>
								</div>
								</c:if>
							</div>
						</div>
