...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        eventBindAdministrationZoneView();
        initAdministrationZoneView();

    });

    function eventBindAdministrationZoneView() {
        ...
        $("#code1").on("change", function () {
            cmmUtil.commonnessCodeList($("#code1").val(), "code2");
        });
        var $drop = $("#dragArea");
        $drop.on("dragenter", function (e) { //드래그 요소가 들어왔을떄
            ...
            if (!fileTypes.includes(ext.toLowerCase())) {
                toastr.warning(ext + "의 파일은 업로드 할 수 없습니다.");
                return;
            }
            for (var i = 0; i < files.length; i++) {
                if (uploadFiles.length > maxFileCnt) {
                    toastr.warning("첨부파일은 최대 " + maxFileCnt + "개까지 업로드 가능합니다.");
                    return;
                } else {
    ...
    function initAdministrationZoneView() {
    ...
    <div class="left-popup-body territory-regist-body">
        <form:form commandName="administrationZone" method="post">
            ...
            <div class="position-bottom btn-wrap">
                <div>
                    <button type="button" class="btn basic bi-write2" onClick="fn_insert_administrationZone(this.form)">
                        등록
                    </button>
                </div>
            </div>
