							...
							<div class="bbs-top">
								...
								<div>
									<select name="code1" class="form-select" onChange="fn_select_list();">
										...
										<option value="<c:out value="${result.code}" />" <c:if test="${searchVO.code1 eq result.code}">selected="selected"</c:if>><c:out value="${result.codeNm}" /></option>
										</c:forEach>
									</select>
								</div>
								</form:form>
								<div><button type="button" class="btn basic bi-all" onClick="fn_select_all_list()">전체 보기</button></div>
							</div>
