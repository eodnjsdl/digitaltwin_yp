...
<div class="lnb-body">
    <div class="srch-box">
        <form id="themaMap-form" method="post">
            <div class="form-row">
                <div class="col">
                    <input type="text" name="searchKeyword" class="form-control" placeholder="주제도명 검색" list="tmap-options" autocomplete="off">
                ...
                <div class="col-auto"><button type="submit" class="btn type01 search btn-search">검색</button></div>
            </div>
        </form>
    </div>
                        ...
                        <c:forEach var="themaMapVO" items="${themaMapList}" varStatus="status">
                            <c:choose>
                                <c:when test="${prevClCode ne themaMapVO.themamapClCode}">
                                    ...
                                    <li><span class="form-checkbox"><input type="checkbox" class="tm tm-chk-all" id="theme${themaMapVO.themamapClCode}"><label for="theme${themaMapVO.themamapClCode}">${themaMapVO.themamapCl}</label></span>
                                        <ul class="theme-list-dep2">
                                            <li>
                                                <span class="form-checkbox">
                                                    <input type="checkbox" class="tm tm-chk" id="theme${themaMapVO.themamapClCode}-${count}" data-layer_nm="${themaMapVO.layerNm}">
...
function tmLayerCheckedInit() {
    ...
    $tmChk.each(function(index, item){
        ...
        if (mapType == '2D') { //2d
            item.checked = store.findLayerId(layerNm)? true : false;
