...
<%--                 <input type="hidden" name="memoId" id="memoId" value="<c:out value="${result.memoId}" />"> --%>
            ...
            <div class="position-bottom btn-wrap">
                ...
                <div class="position-absolute right"><button type="button" class="btn basic bi-edit" data-popup="top-popup04-update">수정</button> <button type="button" class="btn basic bi-delete">삭제</button></div>
            </div>
            ...
            <div class="bbs-detail-util">
                <c:if test="${null ne result.prevSj}">
                    <div class="items">
                        ...
                        <div class="desc"><a href="javascript:selectMemoInfoView('<c:out value="${result.prevMemoId}" />')"><c:out value="${result.prevSj}" /></a></div>
                    </div>
                </c:if>
                <c:if test="${null ne result.nextSj}">
                    <div class="items">
                        <div class="term">다음</div>
                        <div class="desc"><a href="javascript:selectMemoInfoView('<c:out value="${result.nextMemoId}" />');"><c:out value="${result.nextSj}" /></a></div>
                    </div>
                </c:if>
            </div>
        </div>
