...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if(resultMsg){
			//alert(resultMsg);
			if(resultMsg == "failStep1"){
				alert("건축물대장목록 조회 오류");
			}else if(resultMsg == "failStep2"){
				alert("층별현황 조회 오류");
			}else if(resultMsg == "failStep3"){
				alert("관련지번현황 조회 오류");
			}else if(resultMsg == "failStep1xml" 
				 ||	 resultMsg == "failStep2xml"
				 ||	 resultMsg == "failStep3xml"
				){
				
				var errMsg 			= "${errMsg}";
				var returnAuthMsg	= "${returnAuthMsg}";
				if(errMsg && returnAuthMsg){
					alert(errMsg+" > "+returnAuthMsg);
				}
