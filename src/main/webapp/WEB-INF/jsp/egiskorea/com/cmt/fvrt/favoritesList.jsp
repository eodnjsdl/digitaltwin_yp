...
<!-- top > 즐겨찾기 -->
    ...
    <div class="popup-body">
        ...
        <div class="tool-popup-body">
            <form:form name="searchFormFavorites" id="searchFormFavorites" method="post" onsubmit="fn_select_favorites_list(); return false;">		
					<input type="hidden" name="pageIndex" value="<c:out value='${searchVO.pageIndex}' />">
            <div class="srch-box">
                <div class="form-row">
                    <div class="col"><input type="text" class="form-control" name="searchWrd" value="<c:out value="${searchVO.searchWrd}"/>"></div>
                    <div class="col-auto"><button type="submit" class="btn type01 search">조회</button></div>
                </div>
            </div>

            <div class="btn-wrap justify-content-end">
                <div><button type="button" id="write_FVRT" class="btn bi-write">등록</button></div>
            </div>
            ...
            <div class="bbs-top marT10">
                ...
                <div class="list-sort">
                    <span class="form-radio text group">
                        <span><input type="radio" name="sortKind" id="favoritesOrder1"  value="0" <c:if test="${searchVO.sortKind == '0'}">checked</c:if>/><label for="favoritesOrder1">제목순</label></span>
                        <span><input type="radio" name="sortKind" id="favoritesOrder2" value="1"  <c:if test="${searchVO.sortKind == '1'}">checked</c:if>/><label for="favoritesOrder2">최신순</label></span>            
                    </span>
                </div>
            </div>
            </form:form>
                            ...
                            <tbody>
                                <c:forEach items="${resultList}" var="result" varStatus="status">
									...
									<tr>
                                        ...
                                        <td class="subject"><a href="javascript:selectFavoriteInfoView('<c:out value="${result.bkmkId}" />');" class="badge-subject"><c:out value="${result.bkmkNm}" />
                                            <c:if test="${bass eq 'y'}">
                                            <span class="badge basic">기본</span></a>
                                            </c:if>
                                        </td>
                                        <td><c:out value="${result.regDt}" /></td>
                                        <td><button type="button"  id="fav_btn" class="icon-btn location" data-x="<c:out value="${result.xcord}"/>"  data-y="<c:out value="${result.ycord}"/>" data-z="<c:out value="${result.cchLevel}"/>" title="이동"></button>
                                            <button type="button" class="icon-btn edit" title="다시저장" onclick="aj_updateFavoritesView('<c:out value="${result.bkmkId}" />')"></button>
<%--                                            <button type="button" class="btn basic bi-delete2" title="삭제" onclick="aj_deleteFavoritesView('<c:out value="${result.bkmkId}" />')">삭제</button>--%>
                ...
                <div class="pagination">
                    <ui:pagination paginationInfo="${paginationInfo}" type="pagination" jsFunction="fn_select_favorites_linkPage"/>
                </div>
            </div>
        </div>
