    ...
    <div class="left-popup-body layerMng-body">
        <div class="tabBoxDepth1-wrap">
            <div class="tabBoxDepth1">
                <ul>
                    <li class="on">
                        <button type="button" id="layerManagement" class="inner-tab leftPopup"
                                data-popup="left-layer-mng">목록관리
                        </button>
                    </li>
                    <li>
                        <button type="button" id="dataConversion" class="inner-tab leftPopup"
                                data-popup="left-layer-mng" onclick="aj_insertDataConversionView();">등록관리
                        </button>
                    </li>
                </ul>
            </div>
            ...
            <div class="tab-cont layerListMng on">
                        ...
                        <div class="search-items-panel">
                                    ...
                                    <div class="col-auto">
                                        <select name="searchCondition" id="" class="form-select">
                                            ...
                                            <c:forEach var="result" items="${layerClassification}" varStatus="status">
                                                <option value="<c:out value="${result.code}"/>"><c:out
                                                        value="${result.codeNm}"/></option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                    <div class="col"><input type="text" name="searchKeyword" class="form-control"
                                                            onKeypress="javascript:if(event.keyCode==13) aj_selectLayerManagementList()">
                                    </div>
                                    <div class="col-auto">
                                        <button type="button" class="btn btn-xsm type06"
                                                onclick="aj_selectLayerManagementList()">검색
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div class="search-header">
													<span class="form-checkbox">
														<span><input type="checkbox" id="lyrAllChk"><label
                                                                for="lyrAllChk"> 전체 선택</label></span>
													</span>
                            </div>
                                        ...
                                        <ul>
                                            <c:forEach var="result" items="${layerManagementList}" varStatus="status">
                                                <li title="<c:out value="${result.dataName}"/>">
																		<span class="form-checkbox">
																			<input type="checkbox"
                                                                                   name="layerMngItem_<c:out value="${result.dataid}"/>"
                                                                                   id="layerMngItem_<c:out value="${result.dataid}"/>">
																			<label for="layerMngItem_<c:out value="${result.dataid}"/>">
																				<img src="" alt="">
																				<span class="layeMng-list-nm"><c:out
                                                                                        value="${result.dataName}"/></span>
																			</label>
																		</span>
                ...
                <div class="position-bottom btn-wrap">
                    <div class="position-absolute left">
                        ...
                        <button type="button" class="btn basic bi-delete" onclick="aj_deleteLayerManagementInfo()">
                            선택삭제
                        </button>
                    </div>
                    <div>
                        <button type="button" class="btn basic bi-check" onclick="aj_insertLayerListInfo()">적용</button>
                    </div>
                </div>
            </div>
            <!-- //목록관리 -->
