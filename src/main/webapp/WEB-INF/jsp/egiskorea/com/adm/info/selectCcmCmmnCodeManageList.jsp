...
function fnSearch(){
	...
	document.searchForm.action = "<c:url value='/com/mngr/info/selectCcmCmmnCodeManageList.do'/>";
	document.searchForm.submit();
}

function fnGoInsert(){
	document.searchForm.action = "<c:url value='/com/mngr/info/insertCcmCmmnCodeManageView.do'/>";
	document.searchForm.submit();
}
...
function fnInquireCodedetail(codeId){
	document.listForm.codeId.value = codeId;
  	document.listForm.action = "<c:url value='/com/mngr/info/selectCcmCmmnCodeManage.do'/>";
										...
										<form:form commandName="searchVO" name="searchForm" method="post" onsubmit="fnSearch(); return false;">
											...
											<div class="desc">
												<form:select cssClass="form-select" path="searchCondition">
													...
													<option value="3"  <c:if test="${searchVO.searchCondition == '3'}">selected="selected"</c:if> >분류코드</option>
													<option value="4"  <c:if test="${searchVO.searchCondition == '4'}">selected="selected"</c:if> >분류코드명</option>
												</form:select>
									...
									<table class="bbs-list">
										...
										<c:forEach var="resultInfo" items="${resultList}" varStatus="status">
											<tr>
												...
												<td><c:out value="${resultInfo.clCodeNm}" escapeXml="false"/></td>
												<td><a href="#" onclick="fnInquireCodedetail('<c:out value='${resultInfo.codeId}'/>');"><c:out value="${resultInfo.codeId}" /></a></td>
												<td><a href="#" onclick="fnInquireCodedetail('<c:out value='${resultInfo.codeId}'/>');"><c:out value="${resultInfo.codeIdNm}" escapeXml="false" /></a></td>
												<td>
													<c:choose>
