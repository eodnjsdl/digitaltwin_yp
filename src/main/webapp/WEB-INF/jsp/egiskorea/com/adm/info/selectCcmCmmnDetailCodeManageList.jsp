...
function fnSearch(){
	...
	document.searchForm.action = "<c:url value='/com/mngr/info/selectCcmCmmnDetailCodeManageList.do'/>";
	document.searchForm.submit();
}

function fnGoInsert(){
	document.searchForm.action = "<c:url value='/com/mngr/info/insertCcmCmmnDetailCodeManageView.do'/>";
	document.searchForm.submit();
}
...
function fnInquireCodedetail(codeId, code){
	document.listForm.codeId.value = codeId;
	document.listForm.code.value = code;
  	document.listForm.action = "<c:url value='/com/mngr/info/selectCcmCmmnDetailCodeManage.do'/>";
										...
										<form:form commandName="searchVO" name="searchForm" method="post" onsubmit="fnSearch(); return false;">
											...
											<div class="desc">
												<form:select cssClass="form-select" path="searchCondition">
													...
													<option value="2"  <c:if test="${searchVO.searchCondition == '2'}">selected="selected"</c:if> >상세코드</option>
													<option value="3"  <c:if test="${searchVO.searchCondition == '3'}">selected="selected"</c:if> >상세코드명</option>
												</form:select>
									...
									<table class="bbs-list">
										...
										<c:forEach var="resultInfo" items="${resultList}" varStatus="status">
											<tr>
												...
												<td><a href="#" onclick="fnInquireCodedetail('<c:out value='${resultInfo.codeId}'/>', '<c:out value='${resultInfo.code}'/>');"><c:out value='${resultInfo.codeNm}' escapeXml="false"/></a></td>
												<td>
													<c:choose>
