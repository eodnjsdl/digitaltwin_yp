...
<%
/**
 * @file Name : selectQnaManageList.jsp
 * @Description : Q&A 관리 목록 페이지
...
<script>
	...
	function fnSearch(){
		...
		document.searchForm.action = "<c:url value='/com/mngr/info/selectQnaManageList.do'/>";
	...
	function fnInquireQnaDetail(qaId) {
		...
		submit({
			action: '<c:url value="/com/mngr/info/selectQnaManage.do"/>',
			method: 'post',
			formData : {
				'qaId': qaId,
	...
	function fncSelDeleteList() {
		if(fncManageChecked()) {
			if(confirm("<spring:message code="common.delete.msg" />")){	//삭제하시겠습니까?
				document.listForm.action = "/com/mngr/info/deleteQnaManageList.do";
