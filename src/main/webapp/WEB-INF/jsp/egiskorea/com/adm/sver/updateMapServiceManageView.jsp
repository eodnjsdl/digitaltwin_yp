...
<%
/**
 * @file Name : updateMapServiceManageView.jsp
 * @Description : 지도서비스 관리 수정 페이지
...
<section id="content">
    <div class="row">
        <div class="col-12 page-tit-wrap">
            ...
            <p class="page-txt">지도서비스를 수정하는 페이지 입니다.</p>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <form:form modelAttribute="mapServiceVO" id="mapService-form" method="post" enctype="multipart/form-data" action="/com/mngr/sver/updateMapServiceManage.do">
                ...
                <div class="bbs-write-default">
                    <table class="bbs-write">
                        ...
                        <tbody>
                                ...
                                <td>
                                    ...
                                    <div class="attached-file">
                                        <c:if test="${not empty mapServiceVO.streFileNm && not empty mapServiceVO.originalFileNm}"><%--디비에 파일이름이 존재할 경우--%>
                                            <c:if test="${egis:isFile(mapServiceVO.streFileNm)}"><%--실제로 파일이 존재할 경우--%>
                                                <div class="attached-file-name"><i class="fa-solid fa-image"></i>&nbsp;<c:out value='${mapServiceVO.originalFileNm}'/></div>
                                                <button class="attached-file-delete-btn btn-delete-file" data-uid="${mapServiceVO.mapserviceId}" title="<spring:message code="button.delete"/> 버튼"><i class="fa-solid fa-trash-can"></i></i></button>
                                                <img class="attached-file-preview hide" src="<c:url value='/com/cmm/image.do'/>?attach=<c:out value='${egis:encrypt(mapServiceVO.streFileNm)}'/>" alt="<c:out value='${mapServiceVO.originalFileNm}'/>">
                                            </c:if>
                                        </c:if>
                                    </div>
                            ...
                            <tr>
                                <th scope="col"><spring:message code="comMngrSver.mapSvcVO.useAt"/></th>
                                <td>
                ...
                <div class="btn-wrap justify-content-end">
                    <div>
                        <a href="<c:url value='/com/mngr/sver/selectMapServiceManage.do'/>" id="btn-cancel" class="btn type07" title="<spring:message code="button.reset"/> 버튼"><spring:message code="button.reset"/></a><%-- 취소 --%>
                        <button type="submit" class="btn type02" id="btn-update" title="<spring:message code="button.update"/> 버튼"><spring:message code="button.update"/></button><%-- 저장 --%>
                    </div>
                </div>
            </form:form>
        </div>
...
<script>
    $(function() {
        ...
        /**
         * 등록된 첨부파일이름 hover 이벤트
         */
        $('.attached-file-name').hover(function(e) {
            var $preview = $('.attached-file-preview');
            $preview.removeClass('hide').fadeIn('fast');
        }, function(e) {
            $('.attached-file-preview').hide();
        });
        ...
        $('.btn-delete-file').click(function(e) {
            ...
            if (confirm("<spring:message code="file.delete.msg"/>")) {
                var mapserviceId = $(this).data('uid');

                $.ajax({
                    type : 'POST',
                    url : '<c:url value="/com/mngr/sver/deleteMapServiceManageFile.do"/>',
                    dataType : "json",
                    data : {'mapserviceId': mapserviceId},
                    beforeSend : function(jqXHR, settings) {
                        $('#wrap').waitMe({
                            effect : 'timer',
                            text : '이미지 파일을 삭제 중입니다.',
                            bg : 'rgba(0,0,0,0.5)',
                            color : '#ffffff',
                            textPos : 'vertical',
                        });
